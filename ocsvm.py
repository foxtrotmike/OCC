...
A barebones implementation of One-Class SVM with PyTorch
...
wb = torch.zeros(D_in+1, D_out, device=device, requires_grad=True)
...
for t in range(1000):
  ...
  #loss = hinge(y,y_pred)  
  loss = hinge1c(y_pred)
  obj = C*loss+b+0.5*torch.transpose(w,1,0).mm(w) #empirical loss + regularization
